...
namespace HamstarHelpers.Classes.DataStructures.BTree {
	...
	public class BTree<TK, TP> where TK : IComparable<TK> {
		...
		public BTreeEntry<TK, TP> Search( TK key ) {
		...
		/// <param name="keyToDelete">Key to be deleted.</param>
		private void DeleteInternal( BTreeNode<TK, TP> node, TK keyToDelete ) {
		...
		private void DeleteKeyFromSubtree( BTreeNode<TK, TP> parentNode, TK keyToDelete, int subtreeIndexInNode ) {
			BTreeNode<TK, TP> childNode = parentNode.Children[subtreeIndexInNode];
			...
			if( childNode.HasReachedMinEntries ) {
				...
				BTreeNode<TK, TP> leftSibling = subtreeIndexInNode > 0 ? parentNode.Children[leftIndex] : null;

				int rightIndex = subtreeIndexInNode + 1;
				BTreeNode<TK, TP> rightSibling = subtreeIndexInNode < parentNode.Children.Count - 1
		...
		private void DeleteKeyFromNode( BTreeNode<TK, TP> node, TK keyToDelete, int keyIndexInNode ) {
			...
			BTreeNode<TK, TP> predecessorChild = node.Children[keyIndexInNode];
			if( predecessorChild.Entries.Count >= this.Degree ) {
				BTreeEntry<TK, TP> predecessor = this.DeletePredecessor( predecessorChild );
				node.Entries[keyIndexInNode] = predecessor;
			} else {
				BTreeNode<TK, TP> successorChild = node.Children[keyIndexInNode + 1];
				if( successorChild.Entries.Count >= this.Degree ) {
					BTreeEntry<TK, TP> successor = this.DeleteSuccessor( predecessorChild );
		...
		private BTreeEntry<TK, TP> DeletePredecessor( BTreeNode<TK, TP> node ) {
			if( node.IsLeaf ) {
		...
		private BTreeEntry<TK, TP> DeleteSuccessor( BTreeNode<TK, TP> node ) {
			if( node.IsLeaf ) {
		...
		private BTreeEntry<TK, TP> SearchInternal( BTreeNode<TK, TP> node, TK key ) {
		...
		private void SplitChild( BTreeNode<TK, TP> parentNode, int nodeToBeSplitIndex, BTreeNode<TK, TP> nodeToBeSplit ) {
			var newNode = new BTreeNode<TK, TP>( this.Degree );
		...
		private void InsertNonFull( BTreeNode<TK, TP> node, TK newKey, TP newPointer ) {
			int positionToInsert = node.Entries.TakeWhile( entry => newKey.CompareTo( entry.Key ) >= 0 ).Count();

			// leaf node
			if( node.IsLeaf ) {
				node.Entries.Insert( positionToInsert, new BTreeEntry<TK, TP>() { Key = newKey, Pointer = newPointer } );
				return;
			}

			// non-leaf
			BTreeNode<TK, TP> child = node.Children[positionToInsert];
