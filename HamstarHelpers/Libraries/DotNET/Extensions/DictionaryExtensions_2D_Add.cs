...
namespace HamstarHelpers.Helpers.DotNET.Extensions {
	...
	public static partial class DictionaryExtensions {
		...
		/// <param name="idx"></param>
		/// <param name="value"></param>
		public static void AddToValue2D<TKey>( this IDictionary<TKey, List<long>> dict, TKey key, int idx, long value ) {
			if( !dict.ContainsKey( key ) ) {
				dict[key] = new List<long>( idx + 1 );
			}
			dict[key][idx] += value;
		}
		/// <summary>
		...
		/// <param name="value"></param>
		public static void AddToValue2D<TKey1, TKey2>(
				this IDictionary<TKey1, IDictionary<TKey2, long>> dict,
				TKey1 key1,
				TKey2 key2,
				int value ) {
			if( !dict.ContainsKey( key1 ) ) {
				dict[key1] = new Dictionary<TKey2, long>();
			}
			if( dict[key1].ContainsKey( key2 ) ) {
				dict[key1][key2] += value;
			} else {
