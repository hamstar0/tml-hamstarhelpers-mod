...
namespace HamstarHelpers.Utilities.Menu {
	[Obsolete( "use Components.UI.MenuItem", true )]
	public class MenuItem : OldMenuItem {
		internal MenuItem( string text, int offset_y, int menu_context, Action my_action ) : base(text, offset_y, menu_context, my_action) {
		}
	}
	...
	public class OldMenuItem {
		public static void AddMenuItem( string text, int offset_y, int menu_context, Action my_action ) {
			...
			var item = new OldMenuItem( text, offset_y, menu_context, my_action );
			string key = text + "." + offset_y + "." + menu_context;
			
			mymod.OldMenuItemMngr.Items[ key ] = item;
		}
		...
		internal OldMenuItem( string text, int offset_y, int menu_context, Action my_action ) {
			this.MenuText = text;
	...
	class OldMenuItemManager {
		internal IDictionary<string, OldMenuItem> Items = new Dictionary<string, OldMenuItem>();
		...
		public OldMenuItemManager() {
			if( !Main.dedServ ) {
				Main.OnPostDraw += OldMenuItemManager._Draw;
			}
		}
		...
		private static void _Draw( GameTime game_time ) {	// <- Just in case references are doing something funky...
			...
			mymod.OldMenuItemMngr.Draw( game_time );
		}

		private void Draw( GameTime game_time ) {
			foreach( OldMenuItem item in this.Items.Values ) {
